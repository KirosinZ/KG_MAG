#include "application_2d.hpp"

#include <vk_boilerplate/boilerplate.hpp>


application_2d::application_2d(const std::string& window_name, const int window_width, const int window_height)
	: title_(window_name)
{
...
void application_2d::init_display()
{
...
void application_2d::init_rendering()
{
	...
	line_shaders_ = boilerplate::shaders(device_, "line_shader");
	point_shaders_ = boilerplate::shaders(device_, "line_shader");
...
void application_2d::set_scene(const scene_2d& scene)
{
	scene_ = scene;
	view_ = view_2d(wnd_.get(), scene_.camera());
...
void application_2d::update_buffers()
{
	const int n_models = scene_.models().size();
	const affine_transform_2d view = scene_.camera().view();

	for (int m_ind = 0; m_ind < n_models; m_ind++)
	{
		std::vector<glm::vec3> new_vertices = scene_.models()[m_ind].current_vertices();

		const int n_vertices = new_vertices.size();
		for (int v_ind = 0; v_ind < n_vertices; v_ind++)
		{
			new_vertices[v_ind] = view * new_vertices[v_ind];
		}

		void* data = vertex_buffers_[m_ind].second.mapMemory(0, n_vertices * sizeof(glm::vec3));
		std::memcpy(data, new_vertices.data(), n_vertices * sizeof(glm::vec3));
